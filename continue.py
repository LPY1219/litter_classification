...
if __name__ == "__main__":
    ...
    if RESUME:
        lr = 1e-3
        Init_Epoch = 0
        optimizer = optim.Adam([{'params':net.parameters(),'initial_lr': lr}],lr)
        path_checkpoint = "./nets/checkpoint/0912-150521/model_epoch_47.pth" #断点路径
    ...
    if Freezing:
        ...
        Cosine_lr = False 
        if Cosine_lr:
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5, eta_min=1e-5,last_epoch=Init_Epoch)
        else:
            lr_scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=1,gamma=0.92,last_epoch=Init_Epoch)
    ...
    if True:
        ...
        Cosine_lr = True
        if Cosine_lr:
            lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=5, eta_min=1e-5,last_epoch=Freeze_Epoch-Freeze_Epoch0+1)
        else:
            lr_scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=1,gamma=0.9,last_epoch=Freeze_Epoch-Freeze_Epoch0+1)
